...
def monte_carlo_outlier_detection(X, y, n_iterations=PARAMS['MCS_N_ITERATIONS'], 
    ...
    print(f"✅ MCS 檢測到 {np.sum(~mask_mcs)} 個異常值，剩剩餘樣本數：{np.sum(mask_mcs)}")
    return mask_mcs
...
for selected_component in components:
    ...
    # 交叉驗證
    cv_scores = cross_val_score(DecisionTreeRegressor(**meta_grid_search.best_params_, random_state=42), 
    ...
    val_cv_scores = cross_val_score(DecisionTreeRegressor(**meta_grid_search.best_params_, random_state=42),
    ...
    test_cv_scores = cross_val_score(DecisionTreeRegressor(**meta_grid_search.best_params_, random_state=42),
    ...
    for name, model in best_models.items():
        if name == 'RF':
            ...
            specific_params['SVM Optimal Gamma'] = model.gamma
    ...
    specific_params['Meta DT Optimal Min Samples Split'] = meta_grid_search.best_params_['min_samples_split']
    ...
    base_model_selection = pd.DataFrame({
        'Parameter/Metric': ['Base Model Selection'],
        'Value': [''],  # 空行作為分隔
        'Category': ['Base Model Selection']
    })
    base_model_selection_data = pd.DataFrame({
        'Parameter/Metric': list(base_model_metrics.keys()),
        'Value': [''] * len(base_model_metrics),
        'Category': ['Base Model Selection'] * len(base_model_metrics),
        'Train R²': [f"{base_model_metrics[name]['train_r2']:.4f}" for name in base_model_metrics],
        'Validation R²': [f"{base_model_metrics[name]['val_r2']:.4f}" for name in base_model_metrics],
        'Test R²': [f"{base_model_metrics[name]['test_r2']:.4f}" for name in base_model_metrics],
        'Selected': [str(name in selected_base_models) for name in base_model_metrics]
    })
    base_model_selection = pd.concat([base_model_selection, base_model_selection_data], ignore_index=True)
...
output_filename = f'All_Components_Meta_Learning_RF_PLS_SVM_DT_Merged_{timestamp}.xlsx'
...
for component in components:
    ...
    if f'{component}_scatter' in all_plot_data:
        ...
        ax.scatter(scatter_data['Y_test_original'], scatter_data['Y_test_pred'], alpha=0.7, color='blue', label='Meta-Learner (DecisionTree)')
