...
# 方差特徵選擇（保留前 100 個高方差波長）
variances = np.var(X_train_sg, axis=0)
top_features = np.argsort(variances)[-100:]
X_train_sg = X_train_sg[:, top_features]
X_test_sg = X_test_sg[:, top_features]
print(f"✅ 方差特徵選擇完成，保留 {len(top_features)} 個波長")
...
print(f"✅ 基於相關性調整 RandomForest max_depth 範圍: {dt_max_depth_range}")
...
# === 步驟 8: 訓練元學習器（SVR） ===
# 根據 meta features 相關性調整 SVM 參數
svm_c_range = [0.1, 1, 10] if avg_meta_corr > 0.7 else [0.1, 1, 10, 100]
svm_epsilon_range = [0.01, 0.1, 0.5]
print(f"✅ 基於 meta features 相關性調整 SVM C 範圍: {svm_c_range}")
meta_param_grid = {
    'kernel': ['rbf', 'linear'],
    'C': svm_c_range,
    'epsilon': svm_epsilon_range
}
meta_svr = SVR()
meta_grid_search = RandomizedSearchCV(meta_svr, meta_param_grid, n_iter=20,
                                     cv=KFold(n_splits=5, shuffle=True, random_state=42),
                                     scoring='r2', n_jobs=-1, refit=True, random_state=42)
meta_grid_search.fit(meta_features_train_pca, Y_train_multi)
best_meta_svr = meta_grid_search.best_estimator_
...
# === 步驟 9: 測試與評估 ===
Y_train_pred_multi = best_meta_svr.predict(meta_features_train_pca)
Y_test_pred_multi = best_meta_svr.predict(meta_features_test_pca)
...
for i, component in enumerate(actual_components):
    ...
    print(f"✅ {component} (SVM) - Train RMSE: {train_rmse:.4f}, Test RMSE: {test_rmse:.4f}")
    print(f"✅ {component} (SVM) - Train R²: {train_r2:.4f}, Test R²: {test_r2:.4f}")
    print(f"✅ {component} (SVM) - SEC: {sec:.4f}, SEP: {sep:.4f}")
...
for i in range(n_components):
    scores = cross_val_score(SVR(**meta_grid_search.best_params_), 
...
for i, component in enumerate(actual_components):
    ...
    ax.scatter(Y_test_multi_original[:, i], Y_test_pred[:, i], alpha=0.7, color='blue', label='Meta-Learner (SVM)')
...
for name, model in best_models.items():
    if name == 'RandomForest':
        ...
        specific_params['RF Optimal Min Samples Leaf'] = model.estimators_[0].min_samples_leaf
    elif name == 'PLS':
...
specific_params['SVM Optimal Kernel'] = meta_grid_search.best_params_['kernel']
specific_params['SVM Optimal C'] = meta_grid_search.best_params_['C']
specific_params['SVM Optimal Epsilon'] = meta_grid_search.best_params_['epsilon']

timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
output_filename = f'Meta_Learning_SVM_OOF_{timestamp}.xlsx'
