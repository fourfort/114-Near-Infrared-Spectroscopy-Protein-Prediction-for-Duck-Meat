...
# 定義統一參數
PARAMS = {
    'WAVELENGTH_RANGE': (750, 1200),  # 波長範圍 (min, max)
    ...
    'TRAIN_TEST_RATIO': 0.25,  # 測試集比例 (0.2 表示 80:20)
...
def snv(X):
    ...
    X: 輸入數據，可以是 DataFrame 或 NumPy 陣列 (樣本數 x 波長數)
    """
    # 如果輸入是 DataFrame，轉換為 NumPy 陣列
...
plt.xlabel('SFA Content (%)', fontsize=14)
plt.ylabel('Frequency', fontsize=14)
plt.title('Distribution of SFA Content', fontsize=16, pad=10)
...
X_train = pd.DataFrame(X_train_snv, index=X_train.index, columns=X_train.columns)
X_test = pd.DataFrame(X_test_snv, index=X_test.index, columns=X_test.columns)
...
base_filename = 'PLS_SFA_SPXY_Standardize_SNV_NoCARS'
counter = 1
