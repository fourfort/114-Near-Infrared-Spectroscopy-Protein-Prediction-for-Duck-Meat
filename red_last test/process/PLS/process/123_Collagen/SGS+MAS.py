...
PARAMS = {
    ...
    'TRAIN_TEST_RATIO': 0.25,  # 測試集比例 (0.25 表示 75:25)
...
def mas(X, window_size=PARAMS['MAS_WINDOW_SIZE']):
    ...
    """
    if window_size % 2 == 0:
        print(f"⚠️ MAS 窗口大小 {window_size} 應為奇數，自動加 1")
        window_size += 1
...
if len(nir_final_df) != 900:
    print(f"⚠️ 波長點數 {len(nir_final_df)} 不等於預期 900，檢查數據格式")
...
if not np.all(collagen_mask):
    print(f"⚠️ 檢測到Collagen含量異常值（應在 0-50%）：{Y[~collagen_mask]}")
    X = X[collagen_mask]
    Y = Y[collagen_mask]
    print(f"✅ 移除Collagen含量異常值後，剩餘樣本數：{X.shape[0]}")
...
base_filename = 'PLS_Collagen_Standardize_SGS_MAS_NoCARS'
counter = 1
