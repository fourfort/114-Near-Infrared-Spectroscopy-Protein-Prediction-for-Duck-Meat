...
PARAMS = {
    ...
    'TRAIN_TEST_RATIO': 0.25,  # 測試集比例 (0.25 表示 80:20)
...
if not np.all(collagen_mask):
    print(f"⚠️ 檢測到膠原含量異常值（應在 0-50%）：{Y[~collagen_mask]}")
    X = X[collagen_mask]
    Y = Y[collagen_mask]
    print(f"✅ 移除膠原異常值後，剩餘樣本數：{X.shape[0]}")
...
X_test_msc = msc(X_test, reference=train_mean_spectrum)
...
print(f"✅ 使用 MSC 數據，訓練集形狀: {X_train_selected.shape}, 測試集形狀: {X_test_selected.shape}")
...
base_filename = 'PLS_Collagen_Standardize_MSC_NoCARS_NoSG'
counter = 1
