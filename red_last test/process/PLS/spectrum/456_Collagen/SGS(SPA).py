...
# === 步驟 3: NIR 光譜標準化（Autoscaling） ===
scaler_x = StandardScaler()
X_scaled = scaler_x.fit_transform(X)
X = pd.DataFrame(X_scaled, index=X.index, columns=X.columns)
print("✅ NIR 光譜 Autoscaling 完成")
...
# === 步驟 5: 光譜預處理（Autoscaling 已在上一步完成） ===
X_train_sg = X_train  # Autoscaling 已完成，無需額外處理
X_test_sg = X_test
print("✅ Autoscaling 預處理完成")

# === 步驟 6: SPA 特徵波長選擇 ===
selected_wavelength_indices = spa(X_train_sg, Y_train, max_wavelengths=PARAMS['SPA_MAX_WAVELENGTHS'])
...
plt.plot(wavelengths, X_train_sg.mean(axis=0), label='Mean Spectrum')
plt.scatter(selected_wavelengths, X_train_sg.mean(axis=0)[selected_wavelength_indices], 
           color='red', marker='o', label='SPA Selected Wavelengths')
plt.xlabel('Wavelength (nm)')
plt.ylabel('Absorbance')
plt.title('SPA Selected Wavelengths on Mean NIR Spectrum (Collagen)')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
plt.title('PLS Regression: Actual vs Predicted Collagen (SPA)')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
output_filename = f'PLS_Collagen_SPA_{timestamp}.xlsx'
