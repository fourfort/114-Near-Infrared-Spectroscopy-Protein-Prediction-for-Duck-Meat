...
PARAMS = {
    ...
    'TRAIN_TEST_RATIO': 0.25,  # 測試集比例 (0.35 表示 65:35)
    'TRAIN_TEST_RATIO_STR': '65:35',  # 訓練-測試比例字串表示
    'CARS_N_FEATURES': 20,  # CARS 特徵波長數量
...
if 'Collagen' in trad_df.columns:
    ...
    print("\n🎯 目標值 Y (Collagen) 前五筆:", Y[:5])
    print("\n🎯 目標值 Y (Collagen) 描述:", pd.Series(Y).describe())
else:
...
print(f"✅ 剔除後目標值 Y (Collagen) 描述:", pd.Series(Y).describe())

# 膠原蛋白含量範圍檢查（假設 0-50%）
collagen_mask = (Y >= 0) & (Y <= 50)
if not np.all(collagen_mask):
    print(f"⚠️ 檢測到膠原蛋白含量異常值（應在 0-50%）：{Y[~collagen_mask]}")
    X = X[collagen_mask]
    Y = Y[collagen_mask]
    print(f"✅ 移除膠原蛋白異常值後，剩餘樣本數：{X.shape[0]}")
    print(f"✅ 移除後目標值 Y (Collagen) 描述:", pd.Series(Y).describe())
...
# === 步驟 5: 光譜預處理（Autoscaling） ===
# Autoscaling 等同於 StandardScaler，已經在步驟 3 完成，因此直接使用 X_train 和 X_test
X_train_scaled = X_train
X_test_scaled = X_test
print("✅ Autoscaling 預處理完成（已在步驟 3 標準化）")
...
selected_wavelengths = cars(X_train_scaled, Y_train)
X_train_selected = X_train_scaled[:, selected_wavelengths]
X_test_selected = X_test_scaled[:, selected_wavelengths]
...
plt.title('CARS Selected Wavelengths for Collagen')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
table.scale(1.2, 1.2)
plt.title('Evaluation Metrics for Training and Test Sets (Collagen)')
plt.show()
...
plt.title('PLS Regression: Actual vs Predicted Collagen')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
output_filename = f'PLS_Collagen_{timestamp}.xlsx'
