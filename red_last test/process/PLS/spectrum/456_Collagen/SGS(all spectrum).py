...
if font_path:
    ...
    print("⚠️ Times New Roman not found, using DejaVu Sans")
...
def safe_r2_score(y_true, y_pred):
    if len(y_true) <= 1 or np.var(y_true) == 0:
        print("⚠️ R² calculation skipped due to insufficient data or zero variance")
        return 0.0
...
plt.xlabel('Collagen Content (%)', fontsize=14)
plt.ylabel('Frequency', fontsize=14)
plt.title('Distribution of Collagen Content', fontsize=16, pad=10)
...
X_train_auto = scaler_auto.fit_transform(X_train)
X_test_auto = scaler_auto.transform(X_test)
print("✅ Autoscaling 預處理完成")

# === 步驟 6: 使用完整波長數據 ===
X_train_selected = X_train_auto
X_test_selected = X_test_auto
...
table.scale(1.2, 1.2)
plt.title('Evaluation Metrics for Training and Test Sets (Collagen)', fontsize=16, pad=10)
plt.tight_layout()
plt.show()
...
plt.xlabel('Actual Collagen Content (%)', fontsize=14)
plt.ylabel('Predicted Collagen Content (%)', fontsize=14)
plt.title('PLS Regression: Actual vs Predicted Collagen', fontsize=16, pad=10)
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
base_filename = 'PLS_Collagen_Standardize_Autoscaling_NoCARS'
counter = 1
