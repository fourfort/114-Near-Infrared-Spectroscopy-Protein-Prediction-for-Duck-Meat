...
if not np.all(salt_mask):
    ...
    X = X[salt_mask]
    Y = Y[salt_mask]
    print(f"✅ 移除鹽含量異常值後，剩餘樣本數：{X.shape[0]}")
    print(f"✅ 移除後目標值 Y 描述:", pd.Series(Y).describe())

# 繪製鹽含量分佈直方圖
plt.figure(figsize=(8, 6))
plt.hist(Y, bins=20, edgecolor='black', alpha=0.7)
plt.xlabel('Salt Content (%)', fontsize=14)
plt.ylabel('Frequency', fontsize=14)
plt.title('Distribution of Salt Content', fontsize=16, pad=10)
...
selected_wavelengths = cars(X_train_sg, Y_train)
...
plt.title('CARS Selected Wavelengths for Salt', fontsize=16, pad=10)
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
table.scale(1.2, 1.2)
plt.title('Evaluation Metrics for Training and Test Sets (Salt)', fontsize=16, pad=10)
plt.tight_layout()
plt.show()
...
plt.xlabel('Actual Salt Content (%)', fontsize=14)
plt.ylabel('Predicted Salt Content (%)', fontsize=14)
plt.title('PLS Regression: Actual vs Predicted Salt', fontsize=16, pad=10)
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
base_filename = 'PLS_Salt_Standardize_SGS_CARS'
counter = 1
