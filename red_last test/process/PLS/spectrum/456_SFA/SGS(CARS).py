...
PARAMS = {
    ...
    'TRAIN_TEST_RATIO': 0.35,  # 測試集比例 (0.35 表示 65:35)
    'TRAIN_TEST_RATIO_STR': '65:35',  # 訓練-測試比例字串表示
    'CARS_N_FEATURES': 15,  # CARS 特徵波長數量
...
if 'SFA' in trad_df.columns:
    ...
    print("\n🎯 目標值 Y (SFA) 前五筆:", Y[:5])
    print("\n🎯 目標值 Y (SFA) 描述:", pd.Series(Y).describe())
else:
...
print(f"✅ 剔除後目標值 Y (SFA) 描述:", pd.Series(Y).describe())

# 飽和脂肪酸含量範圍檢查（0-50%）
sfa_mask = (Y >= 0) & (Y <= 50)
if not np.all(sfa_mask):
    print(f"⚠️ 檢測到飽和脂肪酸含量異常值（應在 0-50%）：{Y[~sfa_mask]}")
    X = X[sfa_mask]
    Y = Y[sfa_mask]
    print(f"✅ 移除飽和脂肪酸異常值後，剩餘樣本數：{X.shape[0]}")
    print(f"✅ 移除後目標值 Y (SFA) 描述:", pd.Series(Y).describe())
...
plt.title('CARS Selected Wavelengths for SFA')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
table.scale(1.2, 1.2)
plt.title('Evaluation Metrics for Training and Test Sets (SFA)')
plt.show()
...
plt.title('PLS Regression: Actual vs Predicted SFA')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
output_filename = f'PLS_SFA_{timestamp}.xlsx'
