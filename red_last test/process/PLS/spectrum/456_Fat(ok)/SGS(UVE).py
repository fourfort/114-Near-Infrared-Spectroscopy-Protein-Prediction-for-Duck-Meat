...
def uve_wavelength_selection(X, y, n_components, n_noise_vars=PARAMS['UVE_NOISE_VARIABLES'], cv_folds=PARAMS['UVE_CV_FOLDS']):
    ...
    # 使用噪聲變量的穩定性閾值選擇重要波長
    threshold = np.percentile(stability_noise, 50)  # 使用 95% 分位數作為閾值
...
def monte_carlo_outlier_detection(X, y, n_iterations=PARAMS['MCS_N_ITERATIONS'], 
    ...
    for souv in range(n_iterations):
...
plt.title('PLS Regression: Actual vs Predicted Fat (UVE)')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
output_filename = f'PLS_Fat_UVE_{timestamp}.xlsx'
