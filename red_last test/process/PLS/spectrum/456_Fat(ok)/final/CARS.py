...
def cars(X, y, n_features=PARAMS['CARS_N_FEATURES'], n_iterations=PARAMS['CARS_N_ITERATIONS']):
    ...
    if n_features >= n_wavelengths:
        print(f"⚠️ n_features ({n_features}) >= n_wavelengths ({n_wavelengths}), returning all indices")
    ...
    for _ in range(n_iterations):
        ...
        if np.sum(coef) == 0:
            print("⚠️ PLS coefficients are all zero, stopping CARS iteration")
            break
    ...
    if not selected_history:
        print("⚠️ No valid iterations in CARS, returning initial indices")
