...
def spa(X, y, max_wavelengths=PARAMS['SPA_MAX_WAVELENGTHS']):
    ...
    if max_wavelengths < 1:
        print("⚠️ max_wavelengths must be at least 1, returning empty list")
        return []
    ...
    if np.all(np.isnan(correlations)):
        print("⚠️ All correlations are NaN, selecting first wavelength")
        first_idx = 0
    else:
    ...
    for _ in range(max_wavelengths - 1):
        ...
        for i, idx in enumerate(remaining_indices):
            ...
            try:
                ...
                projections[i] = 0
                print(f"⚠️ Singular matrix encountered for wavelength index {idx}")
        
        if np.all(projections == 0):
            print("⚠️ All projections are zero, stopping SPA")
            break
