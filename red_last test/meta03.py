...
PARAMS = {
    ...
    'K_FOLDS': 10,
    'PCA_N_COMPONENTS': 5,
    'DT_MAX_DEPTH_RANGE': [7, 10, 12, 15],  # 優化範圍
    'DT_MIN_SAMPLES_SPLIT_RANGE': [2, 5, 10],
    'DT_MIN_SAMPLES_LEAF_RANGE': [5, 10, 20],  # 新增
...
# 檢查 SFA 和 Salt 分佈
print(f"✅ SFA 描述:", pd.Series(Y_multi[:, components.index('SFA')]).describe())
print(f"✅ Salt 描述:", pd.Series(Y_multi[:, components.index('Salt')]).describe())
...
if high_vif:
    ...
    priority_components = ['Moisture', 'Protein', 'Fat', 'SFA', 'Salt']  # 優先保留 SFA 和 Salt
...
print(f"✅ 平均 Pearson 相關性: {avg_corr:.4f}")
...
# 根據相關性調整參數範圍
dt_max_depth_range = [7, 10, 12] if avg_corr > 0.6 else PARAMS['DT_MAX_DEPTH_RANGE']
pls_n_components = [3, 5, 7] if avg_corr > 0.6 else PARAMS['PLS_N_COMPONENTS']
print(f"✅ 基於相關性調整 DecisionTree max_depth 範圍: {dt_max_depth_range}")
print(f"✅ 基於相關性調整 PLS n_components 範圍: {pls_n_components}")
...
base_models = {
    ...
    'DecisionTree': {
        ...
        'param_grid': {
            'estimator__max_depth': dt_max_depth_range,
            'estimator__min_samples_split': PARAMS['DT_MIN_SAMPLES_SPLIT_RANGE'],
            'estimator__min_samples_leaf': PARAMS['DT_MIN_SAMPLES_LEAF_RANGE']
        }
    }
}
...
avg_meta_corr = corr_matrix.abs().mean().mean()
...
# 根據 meta features 相關性調整 PCA 組分數
pca_n_components = min(5, len(selected_features) // 2) if avg_meta_corr > 0.7 else min(10, len(selected_features))
print(f"✅ 基於 meta features 相關性調整 PCA 組分數: {pca_n_components}")
...
# 根據 meta features 相關性調整 DT 參數
dt_max_depth = [3, 5] if avg_meta_corr > 0.7 else [3, 5, 7]
print(f"✅ 基於 meta features 相關性調整 DT max_depth 範圍: {dt_max_depth}")
meta_param_grid = {
    'estimator__max_depth': dt_max_depth,
    'estimator__min_samples_leaf': [1, 2, 4]
}
meta_dt = MultiOutputRegressor(DecisionTreeRegressor(random_state=42))
...
for i, comp in enumerate(components):
    ...
    if valid_cols:
        ...
        print(f"⚠️ 成分 {comp} 無有效 meta features，使用 PLS 預測作為備用")
        pls_idx = list(base_models.keys()).index('PLS')
        avg_preds_train[:, i] = meta_features_train[:, pls_idx * len(components) + i]
        avg_preds_test[:, i] = meta_features_test[:, pls_idx * len(components) + i]

# === 步驟 9: 測試與評估 ===
Y_train_pred_multi = best_meta_dt.predict(meta_features_train_pca)
Y_test_pred_multi = best_meta_dt.predict(meta_features_test_pca)
...
for i, component in enumerate(actual_components):
    ...
    print(f"✅ {component} (DecisionTree) - Train RMSE: {train_rmse:.4f}, Test RMSE: {test_rmse:.4f}")
    print(f"✅ {component} (DecisionTree) - Train R²: {train_r2:.4f}, Test R²: {test_r2:.4f}")
    print(f"✅ {component} (DecisionTree) - SEC: {sec:.4f}, SEP: {sep:.4f}")
...
for i in range(n_components):
    scores = cross_val_score(DecisionTreeRegressor(**{k.split('__')[1]: v for k, v in meta_grid_search.best_params_.items()}), 
...
for i, component in enumerate(actual_components):
    ...
    ax.scatter(Y_test_multi_original[:, i], Y_test_pred[:, i], alpha=0.7, color='blue', label='Meta-Learner (DecisionTree)')
...
for name, model in best_models.items():
    if name == 'DecisionTree':
        specific_params['DT Optimal Max Depth'] = model.estimators_[0].max_depth
        specific_params['DT Optimal Min Samples Split'] = model.estimators_[0].min_samples_split
        specific_params['DT Optimal Min Samples Leaf'] = model.estimators_[0].min_samples_leaf
    elif name == 'SVM':
        specific_params['SVM Optimal Kernel'] = model.estimators_[0].kernel
        specific_params['SVM Optimal C'] = model.estimators_[0].C
        specific_params['SVM Optimal Epsilon'] = model.estimators_[0].epsilon
    elif name == 'PLS':
        specific_params['PLS Optimal N Components'] = model.n_components

specific_params['Meta DT Optimal Max Depth'] = meta_grid_search.best_params_['estimator__max_depth']
specific_params['Meta DT Optimal Min Samples Leaf'] = meta_grid_search.best_params_['estimator__min_samples_leaf']

timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
output_filename = f'Meta_Learning_DecisionTree_OOF_{timestamp}.xlsx'
