...
# ------------------- 評估 -------------------
model.eval(); preds_scaled, trues_scaled = [], []
with torch.no_grad():
...
plt.xlabel('Actual Protein (%)'); plt.ylabel('Predicted Protein (%)')
plt.title('1‑D CNN Prediction'); plt.grid(True); plt.tight_layout(); plt.show()
