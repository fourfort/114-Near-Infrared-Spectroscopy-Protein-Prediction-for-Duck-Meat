...
# 定義統一參數
PARAMS = {
    'WAVELENGTH_RANGE': (650, 1099.5),  # 波長範圍（600 點）
...
# === 步驟 9: 繪製特徵重要性圖 ===
X_full_scaled = scaler_x.fit_transform(nir_transposed_df.loc[common_ids][mask][fat_mask])
...
plt.title('Top 30 Feature Importance for Fat Prediction')
plt.grid(True)
plt.tight_layout()
plt.show()
...
plt.title('Random Forest Regression: Actual vs Predicted Fat')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
output_filename = f'RF_Fat_Reordered_{timestamp}.xlsx'
