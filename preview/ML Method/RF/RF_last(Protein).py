...
# 定義統一參數
PARAMS = {
    'WAVELENGTH_RANGE': (800, 1099.5),  # 波長範圍（接受 600 點）
    'WAVELENGTH_RANGE_STR': '800-1099.5',
    'MCS_N_ITERATIONS': 200,  # MCS 迭代次數（降低）
    'MCS_N_COMPONENTS': 7,  # MCS 主成分數
    'MAHALANOBIS_THRESHOLD': 65,  # 馬氏距離閾值（放寬）
...
# 多重散射校正（MSC）
def msc(input_data):
    input_data = np.array(input_data)  # 轉為 NumPy 陣列
...
# 蛋白質含量範圍檢查（5-50%）
protein_mask = (Y >= 5) & (Y <= 50)
if not np.all(protein_mask):
    print(f"⚠️ 檢測到蛋白質含量異常值（應在 5-50%）：{Y[~protein_mask]}")
    X = X[protein_mask]
    Y = Y[protein_mask]
...
print("✅ 訓練集和測試集 Savitzky-Golay 濾波完成")
...
# === 步驟 9: 繪製特徵重要性圖 ===
# 使用原始波長數據進行特徵重要性分析
X_full_scaled = scaler_x.fit_transform(nir_transposed_df.loc[common_ids][mask][protein_mask])
X_full_sg = savgol_filter(X_full_scaled, window_length=11, polyorder=2, deriv=0)
X_full_msc = msc(X_full_sg)
X_full_processed = StandardScaler().fit_transform(X_full_msc)
full_rf = RandomForestRegressor(**best_params, random_state=42)
full_rf.fit(X_full_processed, Y_scaled)
...
plt.title('Top 30 Feature Importance for Protein Prediction')
plt.grid(True)
plt.tight_layout()
plt.show()
print("✅ 已顯示並儲存特徵重要性圖（前 30 個波長）")
...
plt.savefig('metrics_table_protein.png', dpi=300, bbox_inches='tight')
plt.show()
...
plt.title('Random Forest Regression: Actual vs Predicted Protein')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig('scatter_plot_protein.png', dpi=300, bbox_inches='tight')
plt.show()
...
output_filename = f'RF_Protein_Reordered_{timestamp}.xlsx'
