...
# 定義統一參數
PARAMS = {
    'WAVELENGTH_RANGE': (750, 1099.5),  # 波長範圍（600 點）
...
if 'Salt' in trad_df.columns:
    ...
    print("\n🎯 目標值 Y (Salt) 前五筆:", Y[:5])
    print("\n🎯 目標值 Y (Salt) 描述:", pd.Series(Y).describe())
else:
...
print(f"✅ 剔除後目標值 Y (Salt) 描述:", pd.Series(Y).describe())

# Salt 含量範圍檢查（0-15%）
salt_mask = (Y >= 0) & (Y <= 15)
if not np.all(salt_mask):
    print(f"⚠️ 檢測到鹽分含量異常值（應在 0-15%）：{Y[~salt_mask]}")
    X = X[salt_mask]
    Y = Y[salt_mask]
    print(f"✅ 移除鹽分異常值後，剩餘樣本數：{X.shape[0]}")
    print(f"✅ 移除後目標值 Y (Salt) 描述:", pd.Series(Y).describe())
...
# === 步驟 9: 繪製特徵重要性圖 ===
X_full_scaled = scaler_x.fit_transform(nir_transposed_df.loc[common_ids][mask][salt_mask])
...
plt.title('Top 30 Feature Importance for Salt Prediction')
plt.grid(True)
plt.tight_layout()
plt.show()
...
plt.title('Random Forest Regression: Actual vs Predicted Salt')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
output_filename = f'RF_Salt_Reordered_{timestamp}.xlsx'
