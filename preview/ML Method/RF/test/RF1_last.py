...
# === 定義統一參數 ===
PARAMS = {
    'WAVELENGTH_RANGE': (800, 1099.5),  # 波長範圍
...
# Salt 含量範圍檢查（0-10%）
Salt_mask = (Y >= 0) & (Y <= 10)
if not np.all(Salt_mask):
    print(f"⚠️ 檢測到鹽含量異常值（應在 0-10%）：{Y[~Salt_mask]}")
    X = X[Salt_mask]
    Y = Y[Salt_mask]
    print(f"✅ 移除鹽異常值後，剩餘樣本數：{X.shape[0]}")
...
plt.title('Distribution of Salt Content')
plt.grid(True)
plt.tight_layout()
plt.show()
...
print(f"✅ 訓練集目標值 Y (Salt) 描述:", pd.Series(scaler_y.inverse_transform(Y_train.reshape(-1, 1)).ravel()).describe())
print(f"✅ 測試集目標值 Y (Salt) 描述:", pd.Series(scaler_y.inverse_transform(Y_test.reshape(-1, 1)).ravel()).describe())
...
plt.title("Top 30 Important Features in SVM for Salt")
plt.tight_layout()
plt.show()
...
plt.xlabel('Actual Salt Content (%)')
plt.ylabel('Predicted Salt Content (%)')
plt.title('SVM Regression: Actual vs Predicted Salt')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
output_filename = f'SVR_Salt_No_CARS_{timestamp}.xlsx'
try:
