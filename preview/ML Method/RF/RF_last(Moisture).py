...
# 定義統一參數
PARAMS = {
    'WAVELENGTH_RANGE': (825, 1099.5),  # 波長範圍（600 點）
    'WAVELENGTH_RANGE_STR': '825-1099.5',
...
if 'Moisture' in trad_df.columns:
    ...
    print("\n🎯 目標值 Y (Moisture) 前五筆:", Y[:5])
    print("\n🎯 目標值 Y (Moisture) 描述:", pd.Series(Y).describe())
else:
...
print(f"✅ 剔除後目標值 Y (Moisture) 描述:", pd.Series(Y).describe())
...
if not np.all(moisture_mask):
    ...
    print(f"✅ 移除後目標值 Y (Moisture) 描述:", pd.Series(Y).describe())
...
# === 步驟 9: 繪製特徵重要性圖 ===
X_full_scaled = scaler_x.fit_transform(nir_transposed_df.loc[common_ids][mask][moisture_mask])
...
plt.title('Top 30 Feature Importance for Moisture Prediction')
plt.grid(True)
plt.tight_layout()
plt.show()
...
plt.title('Random Forest Regression: Actual vs Predicted Moisture')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
output_filename = f'RF_Moisture_Reordered_{timestamp}.xlsx'
