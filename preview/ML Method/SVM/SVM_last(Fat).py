...
if not np.all(Fat_mask):
    print(f"⚠️ 檢測到脂肪含量異常值（應在 0-50%）：{Y[~Fat_mask]}")
    X = X[Fat_mask]
    Y = Y[Fat_mask]
...
plt.title("Top 30 Important Features in SVM for Fat")
plt.tight_layout()
plt.show()
...
plt.title('SVM Regression: Actual vs Predicted Fat')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
output_filename = f'SVR_Fat_No_CARS_{timestamp}.xlsx'
try:
