...
# === 定義統一參數 ===
PARAMS = {
    'WAVELENGTH_RANGE': (700, 1099.5),  # 波長範圍
    'WAVELENGTH_RANGE_STR': '700-1099.5',
    'MCS_N_ITERATIONS': 1000,  # 蒙特卡羅採樣次數
    'MCS_N_COMPONENTS': 10,  # PLS降維組分數
    'MAHALANOBIS_THRESHOLD': 70,  # 馬氏距離閾值
...
# === 步驟 1: 數據獲取與處理 ===
try:
    nir_df = pd.read_csv(r'D:\Users\wedke\document\extra document\大三上\113專題研究之製作\論文人生\紅外線\red_coding main\model_train\nir_data.csv')
...
try:
    trad_df = pd.read_excel(r'D:\Users\wedke\document\extra document\大三上\113專題研究之製作\論文人生\紅外線\red_coding main\model_train\trad_data.xlsx')
    print("\n✅ trad_data.xlsx 前五行:")
    print(trad_df.head())
except FileNotFoundError:
...
# Protein 含量範圍檢查（0-70%）
Protein_mask = (Y >= 0) & (Y <= 70)
if not np.all(Protein_mask):
    print(f"⚠️ 檢測到蛋白質含量異常值（應在 0-70%）：{Y[~Protein_mask]}")
    X = X[Protein_mask]
    Y = Y[Protein_mask]
...
print(f"✅ 訓練集目標值 Y (Protein) 描述:", pd.Series(scaler_y.inverse_transform(Y_train.reshape(-1, 1)).ravel()).describe())
print(f"✅ 測試集目標值 Y (Protein) 描述:", pd.Series(scaler_y.inverse_transform(Y_test.reshape(-1, 1)).ravel()).describe())
...
plt.title("Top 30 Important Features in SVM for Protein")
plt.tight_layout()
plt.show()
...
plt.title('SVM Regression: Actual vs Predicted Protein')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
output_filename = f'SVR_Protein_No_CARS_{timestamp}.xlsx'
try:
