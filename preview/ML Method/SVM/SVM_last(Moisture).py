...
PARAMS = {
    ...
    'MAHALANOBIS_THRESHOLD': 85,  # 馬氏距離閾值
...
# Moisture 含量範圍檢查（0-100%）
Moisture_mask = (Y >= 0) & (Y <= 100)
if not np.all(Moisture_mask):
    print(f"⚠️ 檢測到水分含量異常值（應在 0-100%）：{Y[~Moisture_mask]}")
    X = X[Moisture_mask]
    Y = Y[Moisture_mask]
...
print(f"✅ 訓練集目標值 Y (Moisture) 描述:", pd.Series(scaler_y.inverse_transform(Y_train.reshape(-1, 1)).ravel()).describe())
print(f"✅ 測試集目標值 Y (Moisture) 描述:", pd.Series(scaler_y.inverse_transform(Y_test.reshape(-1, 1)).ravel()).describe())
...
plt.title("Top 30 Important Features in SVM for Moisture")
plt.tight_layout()
plt.show()
...
plt.title('SVM Regression: Actual vs Predicted Moisture')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
output_filename = f'SVR_Moisture_No_CARS_{timestamp}.xlsx'
try:
