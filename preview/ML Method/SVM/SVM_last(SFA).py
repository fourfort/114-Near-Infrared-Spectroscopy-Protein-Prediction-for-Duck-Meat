...
if 'SFA' in trad_df.columns:
    trad_df = trad_df.dropna(subset=['SFA'])
    Y = trad_df['SFA'].values
    print("\n🎯 目標值 Y 前五筆:", Y[:5])
    print("\n🎯 目標值 Y 描述:", pd.Series(Y).describe())
else:
    print("❌ 缺少 'SFA' 欄位，無法建立模型")
    exit()
...
# SFA 含量範圍檢查（0-50%）
SFA_mask = (Y >= 0) & (Y <= 50)
if not np.all(SFA_mask):
    print(f"⚠️ 檢測到SFA含量異常值（應在 0-50%）：{Y[~SFA_mask]}")
    X = X[SFA_mask]
    Y = Y[SFA_mask]
    print(f"✅ 移除SFA異常值後，剩餘樣本數：{X.shape[0]}")
    print(f"✅ 移除後目標值 Y 描述:", pd.Series(Y).describe())

# 繪製SFA含量分佈直方圖
plt.figure(figsize=(8, 6))
plt.hist(Y, bins=20, edgecolor='black')
plt.xlabel('SFA Content (%)')
plt.ylabel('Frequency')
plt.title('Distribution of SFA Content')
plt.grid(True)
plt.tight_layout()
plt.show()
...
print(f"✅ 訓練集目標值 Y (SFA) 描述:", pd.Series(scaler_y.inverse_transform(Y_train.reshape(-1, 1)).ravel()).describe())
print(f"✅ 測試集目標值 Y (SFA) 描述:", pd.Series(scaler_y.inverse_transform(Y_test.reshape(-1, 1)).ravel()).describe())
...
plt.title("Top 30 Important Features in SVM for SFA")
plt.tight_layout()
plt.show()
...
plt.xlabel('Actual SFA Content (%)')
plt.ylabel('Predicted SFA Content (%)')
plt.title('SVM Regression: Actual vs Predicted SFA')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
output_filename = f'SVR_SFA_No_CARS_{timestamp}.xlsx'
try:
