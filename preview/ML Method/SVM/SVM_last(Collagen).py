...
if 'Collagen' in trad_df.columns:
    trad_df = trad_df.dropna(subset=['Collagen'])
    Y = trad_df['Collagen'].values
    print("\n🎯 目標值 Y 前五筆:", Y[:5])
    print("\n🎯 目標值 Y 描述:", pd.Series(Y).describe())
else:
    print("❌ 缺少 'Collagen' 欄位，無法建立模型")
    exit()
...
np.random.seed(42)
md_thresholds = []
for _ in range(PARAMS['MCS_N_ITERATIONS']):
...
# Collagen 含量範圍檢查（0-30%）
Collagen_mask = (Y >= 0) & (Y <= 30)
if not np.all(Collagen_mask):
    print(f"⚠️ 檢測到膠原蛋白含量異常值（應在 0-30%）：{Y[~Collagen_mask]}")
    X = X[Collagen_mask]
    Y = Y[Collagen_mask]
    print(f"✅ 移除膠原蛋白異常值後，剩餘樣本數：{X.shape[0]}")
    print(f"✅ 移除後目標值 Y 描述:", pd.Series(Y).describe())

# 繪製膠原蛋白含量分佈直方圖
plt.figure(figsize=(8, 6))
plt.hist(Y, bins=20, edgecolor='black')
plt.xlabel('Collagen Content (%)')
plt.ylabel('Frequency')
plt.title('Distribution of Collagen Content')
plt.grid(True)
plt.tight_layout()
plt.show()
...
def spxy(X, Y, test_size=PARAMS['TRAIN_TEST_RATIO']):
    ...
    dist_XY = dist_X + 2 * dist_Y  # 增加目標值權重
    ...
    return X_train, X_test, Y_train, Y_test, selected, remaining

scaler_y = StandardScaler()
Y_scaled = scaler_y.fit_transform(Y.reshape(-1, 1)).ravel()
X_train, X_test, Y_train, Y_test, train_idx, test_idx = spxy(X, Y_scaled)
feature_names = X.columns
print(f"✅ SPXY 訓練集形狀: {X_train.shape}, 測試集形狀: {X_test.shape}")
print(f"✅ 訓練集目標值 Y (Collagen) 描述:", pd.Series(scaler_y.inverse_transform(Y_train.reshape(-1, 1)).ravel()).describe())
print(f"✅ 測試集目標值 Y (Collagen) 描述:", pd.Series(scaler_y.inverse_transform(Y_test.reshape(-1, 1)).ravel()).describe())
...
# === 步驟 6: 模型訓練 ===
param_grid = {'C': PARAMS['SVR_C_RANGE'], 'epsilon': PARAMS['SVR_EPSILON_RANGE']}
svm = GridSearchCV(SVR(kernel='rbf', gamma='scale'), param_grid, cv=5, scoring='r2')
start_train_time = time.time()
svm.fit(X_train_sg, Y_train)
train_time = time.time() - start_train_time
best_params = svm.best_params_
print(f"✅ SVM 模型訓練完成，最佳參數: {svm.best_params_}, 訓練時間: {train_time:.2f} 秒")
...
# 5-fold 交叉驗證評估
cv_scores = cross_val_score(svm.best_estimator_, X_train_sg, Y_train, cv=5, scoring='r2')
...
# 計算並繪製特徵重要性
perm_importance = permutation_importance(svm.best_estimator_, X_test_sg, Y_test, n_repeats=10, random_state=42, scoring='r2')
...
top_features = [feature_names_list[i] for i in sorted_idx]
top_importances = [feature_importance[i] for i in sorted_idx]
plt.figure(figsize=(12, 6))
plt.bar(top_features, top_importances)
plt.xticks(rotation=90)
plt.ylabel("Feature Importance")
plt.title("Top 30 Important Features in SVM for Collagen")
plt.tight_layout()
plt.show()
...
plt.xlabel('Actual Collagen Content (%)')
plt.ylabel('Predicted Collagen Content (%)')
plt.title('SVM Regression: Actual vs Predicted Collagen')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
output_filename = f'SVR_Collagen_No_CARS_{timestamp}.xlsx'
try:
