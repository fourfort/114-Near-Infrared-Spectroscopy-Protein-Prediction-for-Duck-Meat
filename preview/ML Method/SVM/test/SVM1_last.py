...
nir_filtered_df = nir_df[(nir_df['Wavelength'] >= 650) & (nir_df['Wavelength'] <= 1099.5)]
...
# === 步驟 2: 去除異常值 ===
pca = PCA(n_components=min(X.shape[0], X.shape[1], 30))  # 增加降維維度
...
for _ in range(n_simulations):
    ...
    md_thresholds.append(np.percentile(md_sim, 80))  # 提高閾值
...
X_train, X_test, Y_train, Y_test = train_test_split(X, Y_scaled, test_size=0.2, random_state=42, stratify=np.digitize(Y_scaled, np.percentile(Y_scaled, [10, 25, 50, 75, 90])))
...
# === 步驟 6: 模型訓練 ===
# 使用 GridSearchCV 優化參數
param_grid = {'C': [100, 500, 1000, 2000, 5000], 'epsilon': [0.001, 0.01, 0.1]}
