...
PARAMS = {
    ...
    'MCS_N_COMPONENTS': 20,  # MCS PLS 主成分數
...
if not np.all(salt_mask):
    print(f"⚠️ 檢測到鹽分含量異常值（應在 0-5%）：{Y[~salt_mask]}")
    X = X[salt_mask]
    Y = Y[salt_mask]
...
plt.title('CARS Selected Wavelengths for Salt')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
output_filename = f'PLS_Salt_{timestamp}.xlsx'
