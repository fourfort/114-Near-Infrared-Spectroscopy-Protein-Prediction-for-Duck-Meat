...
# 定義統一參數
PARAMS = {
    'WAVELENGTH_RANGE': (850, 1000),  # 波長範圍 (min, max)
    'WAVELENGTH_RANGE_STR': '850-1000',  # 波長範圍字串表示
    'MCS_N_ITERATIONS': 1000,  # MCS 抽樣次數
    'MCS_N_COMPONENTS': 10,  # MCS PLS 主成分數
...
# 膠原蛋白含量範圍檢查（0-10%）
collagen_mask = (Y >= 0) & (Y <= 10)
if not np.all(collagen_mask):
    print(f"⚠️ 檢測到膠原蛋白含量異常值（應在 0-10%）：{Y[~collagen_mask]}")
    X = X[collagen_mask]
    Y = Y[collagen_mask]
