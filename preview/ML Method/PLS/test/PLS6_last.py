...
# === 步驟 6: 特徵波長提取（CARS） ===
def cars(X, y, n_features=20, n_iterations=20):
...
selected_wavelengths = cars(X_train_sg, Y_train, n_features=20)
...
# # === 步驟 7: 貝葉斯優化選擇最佳 n_components（使用 LOOCV） ===
...
#         y_pred_loo = pls.predict(X_test_loo)
...
# === [新增] 交叉驗證找最佳 n_components ===
max_lv = min(20, X_train_selected.shape[1], X_train_selected.shape[0])  # 最大可能的 LV 數
rmse_cv = []
...
for lv in range(1, max_lv + 1):
    ...
    rmse_cv.append(-scores.mean())  # 將負的 RMSE 轉正

best_lv = np.argmin(rmse_cv) + 1  # 找出最低 RMSE 對應的 LV
...
#     pls_loo = PLSRegression(n_components=best_lv)
#     pls_loo.fit(X_train_loo, y_train_loo)
#     y_pred_loo = pls_loo.predict(X_test_loo)
#     r2_loo = r2_score(y_test_loo, y_pred_loo)
#     cv_scores.append(r2_loo)
# cv_scores = np.array(cv_scores)
# print(f"\n✅ LOOCV R² Scores: Mean {np.nanmean(cv_scores):.4f} (± {np.nanstd(cv_scores) * 2:.4f})")
