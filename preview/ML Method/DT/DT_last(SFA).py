...
if not np.all(sfa_mask):
    print(f"⚠️ 檢測到 SFA 含量異常值（應在 0-50%）：{Y[~sfa_mask]}")
    X = X[sfa_mask]
    Y = Y[sfa_mask]
    print(f"✅ 移除 SFA 異常值後，剩餘樣本數：{X.shape[0]}")
...
plt.title('Decision Tree Structure for SFA Prediction (Max Depth 3)')
plt.show()
...
plt.title('Decision Tree Regression: Actual vs Predicted SFA')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
output_filename = f'DT_SFA_{timestamp}.xlsx'
