...
if not np.all(fat_mask):
    print(f"⚠️ 檢測到脂肪含量異常值（應在 0-100%）：{Y[~fat_mask]}")
    X = X[fat_mask]
    Y = Y[fat_mask]
...
plt.title('Decision Tree Structure for Fat Prediction (Max Depth 3)')
plt.show()
...
plt.title('Decision Tree Regression: Actual vs Predicted Fat')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
output_filename = f'DT_Fat_{timestamp}.xlsx'
