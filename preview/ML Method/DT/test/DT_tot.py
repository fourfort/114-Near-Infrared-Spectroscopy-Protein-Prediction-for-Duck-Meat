...
print(f"✅ 最佳參數: max_depth={best_params['estimator__max_depth']}, min_samples_split={best_params['estimator__min_samples_split']}")

# === 步驟 7: 模型訓練 ===
best_multi_dt.fit(X_train_sg, Y_train_multi)
print(f"✅ 模型訓練完成，使用的參數: max_depth={best_params['estimator__max_depth']}, min_samples_split={best_params['estimator__min_samples_split']}")
...
# 指標表格數據（繪圖用）
metrics_data_table = {
    'Component': components * 4,  # 7 成分 × 4 指標
    'Metric': ['RMSE'] * 7 + ['R² Score'] * 7 + ['SEC'] * 7 + ['SEP'] * 7,
    'Training Set': [f'{np.sqrt(mean_squared_error(Y_train_pred[:, i], Y_train_multi_original[:, i])):.4f}' for i in range(7)] +
                    [f'{safe_r2_score(Y_train_pred[:, i], Y_train_multi_original[:, i]):.4f}' for i in range(7)] +
                    [f'{np.sqrt(np.sum((Y_train_multi_original[:, i] - Y_train_pred[:, i]) ** 2) / max(1, n_train - 1)):.4f}' for i in range(7)] +
                    ['N/A'] * 7,
    'Test Set': [f'{np.sqrt(mean_squared_error(Y_test_pred[:, i], Y_test_multi_original[:, i])):.4f}' for i in range(7)] +
                [f'{safe_r2_score(Y_test_pred[:, i], Y_test_multi_original[:, i]):.4f}' for i in range(7)] +
                ['N/A'] * 7 +
                [f'{np.sqrt(np.sum((Y_test_multi_original[:, i] - Y_test_pred[:, i]) ** 2) / (n_test - 1)):.4f}' for i in range(7)]
}
metrics_df_table = pd.DataFrame(metrics_data_table)
print("\n✅ Evaluation Metrics (Table):")
print(metrics_df_table.to_string(index=False))

# 繪製指標表格
fig, ax = plt.subplots(figsize=(12, 8))
ax.axis('off')
table = ax.table(cellText=metrics_df_table.values, colLabels=metrics_df_table.columns, cellLoc='center', loc='center')
table.auto_set_font_size(False)
table.set_fontsize(10)
table.scale(1.2, 1.2)
plt.title('Evaluation Metrics for Multi-Component Prediction')
plt.show()
...
# 繪製預測散點圖（以 Protein 為例）
plt.figure(figsize=(6, 6))
plt.scatter(Y_test_multi_original[:, components.index('Protein')], Y_test_pred[:, components.index('Protein')], alpha=0.7, label='Predicted vs Actual')
plt.plot([min(Y_test_multi_original[:, components.index('Protein')]), max(Y_test_multi_original[:, components.index('Protein')])],
         [min(Y_test_multi_original[:, components.index('Protein')]), max(Y_test_multi_original[:, components.index('Protein')])], 'r--', label='Ideal (y=x)')
plt.xlabel('Actual Protein Content (%)')
plt.ylabel('Predicted Protein Content (%)')
plt.title('Decision Tree Regression: Actual vs Predicted Protein')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()