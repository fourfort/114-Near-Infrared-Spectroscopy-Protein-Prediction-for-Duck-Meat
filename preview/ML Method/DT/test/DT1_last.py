...
if not np.all(salt_mask):
    print(f"⚠️ 檢測到鹽分含量異常值（應在 0-10%）：{Y[~salt_mask]}")
    X = X[salt_mask]
    Y = Y[salt_mask]
    print(f"✅ 移除鹽分異常值後，剩餘樣本數：{X.shape[0]}")
...
plt.title('Decision Tree Structure for Salt Prediction (Max Depth 3)')
plt.show()
print("✅ 已顯示決策樹圖（限制顯示深度為 3）")
...
table.scale(1.2, 1.2)
plt.title('Evaluation Metrics for Training and Test Sets (Salt)')
plt.show()
...
plt.title('Decision Tree Regression: Actual vs Predicted Salt')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
output_filename = f'DT_Salt_{timestamp}.xlsx'
