...
# 定義統一參數
PARAMS = {
    'WAVELENGTH_RANGE': (800, 1099.5),  # 波長範圍 (min, max)
    'WAVELENGTH_RANGE_STR': '800-1099.5',  # 波長範圍字串表示
...
# 膠原蛋白含量範圍檢查（0-15%）
collagen_mask = (Y >= 0) & (Y <= 15)
if not np.all(collagen_mask):
    print(f"⚠️ 檢測到膠原蛋白含量異常值（應在 0-15%）：{Y[~collagen_mask]}")
    X = X[collagen_mask]
    Y = Y[collagen_mask]
...
grid_search = GridSearchCV(dt, param_grid, cv=KFold(n_splits=5, shuffle=True, random_state=42), 
                           scoring='neg_root_mean_squared_error', n_jobs=-1)
grid_search.fit(X_train_sg, Y_train)
best_dt = grid_search.best_estimator_
best_params = grid_search.best_params_
print(f"✅ 最佳參數: max_depth={best_params['max_depth']}, min_samples_split={best_params['min_samples_split']}")

# === 步驟 7: 模型訓練 ===
best_dt.fit(X_train_sg, Y_train)
print(f"✅ 模型訓練完成，使用的參數: max_depth={best_params['max_depth']}, min_samples_split={best_params['min_samples_split']}")

# === 步驟 8: 繪製決策樹 ===
plt.figure(figsize=(25, 15))
from sklearn.tree import plot_tree
plot_tree(best_dt, max_depth=2, feature_names=[f'{w:.1f} nm' for w in nir_final_df['Wavelength'].values], 
          filled=True, rounded=True, fontsize=10)
plt.title('Decision Tree Structure for Collagen Prediction (Max Depth 3)')
plt.show()
print("✅ 已顯示決策樹圖（限制顯示深度為 2）")

# === 步驟 9: 評估結果 ===
Y_train_pred_scaled = best_dt.predict(X_train_sg)
Y_test_pred_scaled = best_dt.predict(X_test_sg)
...
# 5-fold 交叉驗證評估
cv_scores = cross_val_score(best_dt, X_train_sg, Y_train, cv=5, scoring='r2')
...
plt.title('Decision Tree Regression: Actual vs Predicted Collagen')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
output_filename = f'DT_Collagen_{timestamp}.xlsx'
