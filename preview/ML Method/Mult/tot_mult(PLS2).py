...
import seaborn as sns  # 新增：用於熱圖
from statsmodels.stats.outliers_influence import variance_inflation_factor  # 新增：VIF 檢查
...
PARAMS = {
    ...
    'MAHALANOBIS_THRESHOLD': 90,  # 馬氏距離閾值 (%)
    'TRAIN_TEST_RATIO': 0.3,  # 測試集比例 (0.25 表示 75:25)
    'TRAIN_TEST_RATIO_STR': '75:25',  # 訓練-測試比例字串表示
    'CARS_N_FEATURES': 23,  # CARS 特徵波長數量
...
def cars_feature_selection(X, y, n_features=PARAMS['CARS_N_FEATURES'], n_iterations=PARAMS['CARS_N_ITERATIONS'], random_state=42):
    ...
    n_components = y.shape[1]  # 獲取成分數
    selected_features = []
    coef_history = np.zeros(n_wavelengths)  # 初始化為波長數
    ...
    for _ in range(n_iterations):
        ...
        coef = np.abs(pls.coef_)  # 形狀 (n_components, n_wavelengths)
        
        # 對每個成分的係數取平均或總和
        coef_sum = np.sum(coef, axis=0)  # 沿成分軸求和，形狀 (n_wavelengths,)
        coef_history += coef_sum
    
    # 標準化係數歷史
    coef_history /= n_iterations
    
    # 選擇前 n_features 個特徵
    final_features = np.argsort(coef_history)[-n_features:]
...
# 基於 VIF 移除高共線成分 (VIF > 50，寬鬆閾值)
high_vif = vif_data[vif_data['VIF'] > 50]['Component'].tolist()
if high_vif:
    print(f"⚠️ 檢測到高 VIF 成分: {high_vif}")
    # 優先保留關鍵成分，移除次要成分（例如，僅移除 Ash 或 SFA）
    priority_components = ['Moisture', 'Protein', 'Fat']  # 假設這是關鍵成分
    components_to_remove = [c for c in high_vif if c not in priority_components][:1]  # 移除一個高 VIF 非優先成分
...
if Y_multi.shape[1] == 0:
    print("❌ Y_multi 為空，無法繼續。恢復原始成分並禁用 VIF 移除")
...
# === 步驟 6: CARS 特徵選擇 ===
selected_features = cars_feature_selection(X_train_sg, Y_train_multi)
X_train_cars = X_train_sg[:, selected_features]
X_test_cars = X_test_sg[:, selected_features]
print(f"✅ CARS 選擇了 {len(selected_features)} 個特徵波長")

# === 步驟 7: PLS 參數調優 ===
# 基於相關性調整 param_grid (例如，如果平均 |r| 高，減少 n_components 上限)
avg_corr = corr_pearson.abs().mean().mean()
if avg_corr > 0.5:
    max_n_components = min(X_train_cars.shape[1], 15)  # 減少上限以避免過擬合
else:
    max_n_components = min(X_train_cars.shape[1], 20)
param_grid = {
    'n_components': range(2, max_n_components + 1)
}
print(f"✅ 基於相關性調整 param_grid 上限: {max_n_components}")
pls = PLSRegression()
grid_search = GridSearchCV(pls, param_grid, cv=KFold(n_splits=5, shuffle=True, random_state=42),
                           scoring='r2', n_jobs=-1, refit=True)
grid_search.fit(X_train_cars, Y_train_multi)

# 最佳模型與評估
best_pls = grid_search.best_estimator_
best_params = grid_search.best_params_
print(f"✅ 最佳參數: n_components={best_params['n_components']}")

# === 步驟 8: 模型訓練 ===
best_pls.fit(X_train_cars, Y_train_multi)
print(f"✅ 模型訓練完成，使用的參數: n_components={best_params['n_components']}")

# === 步驟 9: 評估結果 ===
Y_train_pred_multi = best_pls.predict(X_train_cars)
Y_test_pred_multi = best_pls.predict(X_test_cars)
...
# 5-fold 交叉驗證評估
cv_scores = cross_val_score(best_pls, X_train_cars, Y_train_multi, cv=5, scoring='r2')
...
for i, component in enumerate(actual_components):
    ...
    plt.title(f'PLS Regression: Actual vs Predicted {component}')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.show()
...
common_params = {
    ...
    'CARS Features': PARAMS['CARS_N_FEATURES'],
    'CARS Iterations': PARAMS['CARS_N_ITERATIONS']
}

# 特有參數
specific_params = {
    'Optimal N Components': best_params['n_components']
}

# 創建 Excel writer
timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
output_filename = f'Multi_PLS_{timestamp}.xlsx'
