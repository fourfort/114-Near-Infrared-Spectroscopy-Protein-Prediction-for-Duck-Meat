...
PARAMS = {
    ...
    'MCS_N_ITERATIONS': 200,  # 減少至 100 次
    ...
    'MAX_DEPTH_RANGE': [3,5,7],  # 縮減範圍
    'MIN_SAMPLES_SPLIT_RANGE': [5,10],  # 縮減範圍
    'N_ESTIMATORS_RANGE': [50, 100],  # 縮減範圍
    'MAX_FEATURES_RANGE': ['auto', 'sqrt']  # 縮減範圍
}
...
def monte_carlo_outlier_detection(X, y, n_iterations=PARAMS['MCS_N_ITERATIONS'], 
    ...
    for _ in range(n_iterations):
        ...
        dt = RandomForestRegressor(max_depth=5, min_samples_split=5, random_state=42, n_estimators=100)
...
# === 步驟 6: PCA 降維 ===
pca = PCA(n_components=10)  # 降至 10 個主成分
...
# === 步驟 7: 隨機森林參數調優 ===
param_grid = {
    'estimator__n_estimators': PARAMS['N_ESTIMATORS_RANGE'],
    'estimator__max_depth': PARAMS['MAX_DEPTH_RANGE'],
    'estimator__min_samples_split': PARAMS['MIN_SAMPLES_SPLIT_RANGE'],
    'estimator__max_features': PARAMS['MAX_FEATURES_RANGE']
}
rf = RandomForestRegressor(random_state=42)
multi_rf = MultiOutputRegressor(rf)
grid_search = GridSearchCV(multi_rf, param_grid, cv=KFold(n_splits=5, shuffle=True, random_state=42),
                           scoring='r2', n_jobs=4, refit=True)  # 限制 n_jobs
grid_search.fit(X_train_pca, Y_train_multi)  # 使用降維後的數據

# 最佳模型與評估
best_multi_rf = grid_search.best_estimator_
best_params = grid_search.best_params_
print(f"✅ 最佳參數: n_estimators={best_params['estimator__n_estimators']}, max_depth={best_params['estimator__max_depth']}, "
      f"min_samples_split={best_params['estimator__min_samples_split']}, max_features={best_params['estimator__max_features']}")

# === 步驟 8: 模型訓練 ===
best_multi_rf.fit(X_train_pca, Y_train_multi)
print(f"✅ 模型訓練完成，使用的參數: n_estimators={best_params['estimator__n_estimators']}, max_depth={best_params['estimator__max_depth']}, "
      f"min_samples_split={best_params['estimator__min_samples_split']}, max_features={best_params['estimator__max_features']}")

# === 步驟 9: 評估結果 ===
Y_train_pred_multi = best_multi_rf.predict(X_train_pca)
Y_test_pred_multi = best_multi_rf.predict(X_test_pca)
...
# 5-fold 交叉驗證評估
cv_scores = cross_val_score(best_multi_rf, X_train_pca, Y_train_multi, cv=5, scoring='r2')
...
for i, component in enumerate(actual_components):
    ...
    plt.title(f'Random Forest Regression: Actual vs Predicted {component}')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.show()
...
specific_params = {
    ...
    'Optimal N Estimators': best_params['estimator__n_estimators'],
    'Optimal Max Features': best_params['estimator__max_features']
}

# 創建 Excel writer
timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
output_filename = f'Multi_RF_{timestamp}.xlsx'
