...
def center(X):
    X = np.array(X)
    means = np.mean(X, axis=0, keepdims=True)  # 沿波長軸中心化
    X_centered = X - means
    return X_centered
...
# === 步驟 5: 光譜預處理（Savitzky-Golay 濾波與中心化） ===
...
X_train_centered = center(X_train_sg)
X_train = pd.DataFrame(X_train_centered, index=X_train.index, columns=X_train.columns)
X_test_centered = center(X_test_sg)
X_test = pd.DataFrame(X_test_centered, index=X_test.index, columns=X_test.columns)
print("✅ Savitzky-Golay 濾波與中心化完成")

# === 步驟 6: 特徵波長提取（CARS） + 統計選中機率 ===
def cars_with_stats(X, y, n_features=PARAMS['CARS_N_FEATURES'], n_iterations=PARAMS['CARS_N_ITERATIONS']):
    n_samples, n_wavelengths = X.shape
    selected_indices = list(range(n_wavelengths))
    weights = np.ones(n_wavelengths)
    selected_history = []
    selection_counts = np.zeros(n_wavelengths, dtype=int)  # 統計每個波長被選中次數
    ...
    for _ in range(n_iterations):
        ...
        selected_indices = np.random.choice(
            selected_indices, size=n_select, replace=False,
    ...
    return final_indices, selection_prob


# 執行 CARS + 統計
selected_wavelengths, selection_prob = cars_with_stats(X_train, Y_train)
...
# 平滑選中機率，增加層次感
selection_prob_smooth = gaussian_filter1d(selection_prob, sigma=8)
...
# 畫熱力背景（Selection Probability）
plt.imshow(
    [selection_prob_smooth],
...
plt.xlabel('Wavelength (nm)')
plt.ylabel('Absorbance (Log(1/R))')
plt.title('Heatmap-style Feature Wavelength Selection Probability')
plt.legend()
